...
/* eslint-disable react-hooks/refs, react-hooks/immutability, react-hooks/preserve-manual-memoization --
   Owner-provided imperative audio/rAF logic (returns refs from custom hooks, mutates <audio>).
   Kept intact on purpose; React Compiler lint rules flag these patterns as false positives. */
// Ported from docs/specs/MusicPlayer.tsx (owner-provided). Core logic intact.
// Additions (marked "SG:"):
//  - optional id / className / children / callbacks / apiRef props
//  - controlled queue loading (startIndex + loadKey + autoPlay) for the global dock
//  - audio focus: one instance plays at a time, only the focus owner handles keyboard shortcuts
//  - rAF work is skipped while the card is offscreen or idle (perf)
//  - small a11y additions (aria-pressed, slider semantics, keyboard zoom)
...
export interface Track {
  id?: string; // SG: optional stable id for "now playing" highlighting
  title: string;
  artist: string;
  cover: string;
  src: string;
...
type Action =
  ...
  | { type: 'CYCLE_LOOP' }
  // SG: load a new queue (global dock)
  | { type: 'RESET_QUEUE'; index: number; trackCount: number; direction: Direction };
...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    ...
    case 'RESET_QUEUE': {
      const order = state.shuffled
        ? shuffleOrder(action.index, action.trackCount)
        : Array.from({ length: action.trackCount }, (_, i) => i);
      return {
        ...state,
        order,
...
function useAudioPlayer(
  ...
  // SG: original loaded tracks[0] on [tracks]; now honours startIndex/loadKey so a
  // host (the global dock) can swap the queue and autoplay a chosen track.
  ...
  useEffect(() => {
    ...
    dispatch({
      ...
      direction: isInitial ? null : 'next',
    });
    audio.src = tracks[index].src;
    audio.load();
    if (autoPlay && loadKey > 0) audio.play().catch(() => {});
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [tracks, startIndex, loadKey]);
...
function useKeyboardShortcuts(actions: ShortcutActions, ownerId: string) {
  useEffect(() => {
    const handler = (e: KeyboardEvent) => {
      const target = e.target as HTMLElement;
      if (target.tagName === 'INPUT') return;
      // SG: only one instance reacts; ignore editable fields, modifier combos & open dialogs
      if (!audioFocus.ownsShortcuts(ownerId)) return;
      ...
      if (
        e.key === ' ' &&
        (target.tagName === 'BUTTON' || target.tagName === 'A' || target.getAttribute('role') === 'button')
      )
        return;
      switch (e.key) {
        case ' ':
...
function ScalesMixer({
  ...
  const idleFrames = useRef(0); // SG: perf

  useRafLoop((_, dt) => {
    // SG: skip DOM writes when offscreen, or once settled after pausing
    if (visibleRef && !visibleRef.current) return;
    if (isPlaying) idleFrames.current = 0;
    else if (++idleFrames.current > 90) return;
...
function Disc({
  ...
  const lastWritten = useRef(NaN); // SG: perf
  ...
  useRafLoop((now) => {
    ...
    if (!el) return;
    if (visibleRef && !visibleRef.current) return; // SG
  ...
  return (
    <div
      ...
      role="button"
      tabIndex={0}
      aria-label={isZoomed ? 'Zoom out cover art' : 'Zoom in cover art'}
      ...
      {layers.map((l, i) => {
        ...
        return (
          ...
          >
            ...
            <p className={`track ${state}`} style={style}>
              {l.track.title}
            </p>
          </div>
        );
      ...
      <div
        ...
        onKeyDown={(e) => {
          ...
          if (e.key === 'ArrowRight' || e.key === 'ArrowLeft') {
            ...
            onSeek(Math.max(0, Math.min(1, currentTime / duration + delta)));
          }
        }}
        onClick={(e) => {
...
export interface MusicPlayerProps {
  ...
  /** SG: audio-focus id. Instances with the same id are treated as one. */
  id?: string;
  className?: string;
  label?: string;
  /** SG: controlled loading — bump loadKey to (re)load tracks[startIndex]. */
  startIndex?: number;
  loadKey?: number;
  autoPlay?: boolean;
...
export function MusicPlayer({
  ...
  const player = useAudioPlayer(tracks, startIndex, loadKey, autoPlay);
  ...
  const [layers, setLayers] = useState<Layer[]>(() => [
    { id: 0, track: tracks[startIndex] ?? tracks[0], dir: null },
  ]);
  // SG: key on index *and* loadKey so swapping queues also swaps the cover layer
  const trackSig = player.state.currentIndex + loadKey * 10000;
  const lastSig = useRef(trackSig);
  const idRef = useRef(1);

  useEffect(() => {
    if (trackSig === lastSig.current) return;
    lastSig.current = trackSig;
  ...
  }, [trackSig, player.currentTrack, player.state.direction]);
  ...
  useKeyboardShortcuts(shortcuts, focusId);

  /* SG: audio focus — pause other instances when this one starts */
  useEffect(() => {
    const audio = player.audioRef.current;
    if (!audio) return;
    const unregister = audioFocus.register(focusId, () => audio.pause());
    const onPlay = () => audioFocus.claim(focusId);
    audio.addEventListener('play', onPlay);
    return () => {
      audio.removeEventListener('play', onPlay);
      audio.pause();
  ...
  }, [focusId, player.audioRef]);

  /* SG: visibility gating for rAF work */
  useEffect(() => {
  ...
  useEffect(() => {
    onPlayingChangeRef.current?.(player.state.isPlaying);
  }, [player.state.isPlaying]);
  useEffect(() => {
    onTrackChangeRef.current?.(player.state.currentIndex, player.currentTrack);
  }, [player.state.currentIndex, player.currentTrack]);

  useImperativeHandle(
    apiRef,
    () => ({
      toggle: player.toggle,
      play: () => player.audioRef.current?.play().catch(() => {}),
      pause: () => player.audioRef.current?.pause(),
    }),
    [player.toggle, player.audioRef]
  );
