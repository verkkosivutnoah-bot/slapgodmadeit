...
// background (≥ ~4.5:1), or null when the cover has no usable color (callers fall back to coral).
  ...
  const p = new Promise<string | null>((resolve) => {
    ...
    img.onload = () => {
      try {
        ...
        // vivid but legible on ink: strong saturation, mid-high lightness (≥ ~5:1 on #1C1917)
        const s = Math.max(0.62, Math.min(0.92, (best.s / best.w) * 1.05));
        const l = 0.66;
...
/** Style object exposing the accent as --track-accent (omitted when unknown → CSS falls back to coral). */
